...
const SupplierList: React.FC = () => {
  ...
  // Update submitRating function to keep the ratings visible after submission
  ...
  const renderRatingSection = (supplier: Supplier) => {
    ...
    const userRating = userRatings[supplier.id];
          ...
          <div className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-100">
            ...
            <div className="space-y-3">
              {enabledAreas.map((area) => {
                ...
                const userPreviousRating = userRating?.[area.id];
  ...
  const toggleDescription = (supplierId: string) => {
  ...
  // Add this function to render truncated description
  const renderDescription = (supplier: Supplier) => {
    const isExpanded = expandedDescriptions[supplier.id];
    const maxLength = 150; // Maximum characters to show before truncation
    
    if (!supplier.full_desc) return null;
    
    const shouldTruncate = supplier.full_desc.length > maxLength;
    const displayText = isExpanded ? supplier.full_desc : supplier.full_desc.slice(0, maxLength) + '...';
    
    return (
      <div className="space-y-2 w-full">
        <div className="bg-gray-50 rounded-lg p-4 w-full">
          <p className="text-gray-700 leading-relaxed break-words whitespace-pre-wrap overflow-hidden w-full">
            {displayText}
          </p>
        </div>
        {shouldTruncate && (
          <Button
            ...
            onClick={() => toggleDescription(supplier.id)}
          >
  ...
  // Add this function to fetch user ratings
  const fetchUserRatings = async (supplierId: string) => {
    if (!user) return;
    ...
    try {
      ...
      if (data) {
        setUserRatings(prev => ({
          ...prev,
          [supplierId]: data.ratings
        }));
        // Set the ratings in the ratings state
        setRatings(data.ratings);
        // Set the feedback if it exists
        if (data.ratings.feedback) {
          setFeedback(data.ratings.feedback);
        }
      }
    } catch (error) {
  ...
  // Modify the setSelectedSupplier to fetch user ratings
  const handleSupplierSelect = (supplier: Supplier) => {
    setSelectedSupplier(supplier);
    fetchUserRatings(supplier.id);
  };
                ...
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <CardTitle className="text-lg break-words">{supplier.org_name}</CardTitle>
              ...
              <CardContent className="pb-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-start space-x-2 text-sm text-gray-600">
                    <span className="font-medium min-w-[100px] flex-shrink-0">Based in:</span>
                    <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-start space-x-2 text-sm text-gray-600">
                    <span className="font-medium min-w-[100px] flex-shrink-0">Operates in:</span>
                    <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-start space-x-2 text-sm text-gray-600">
                    <span className="font-medium min-w-[100px] flex-shrink-0">States covered:</span>
                    <div className="min-w-0 flex-1">
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <DialogTitle className="text-2xl break-words">{selectedSupplier.org_name}</DialogTitle>
              ...
              <div className="space-y-8 mt-6">
                ...
                <div className="w-full">
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg mb-4 w-full">
                    <p className="font-medium text-blue-900 break-words whitespace-pre-wrap w-full">{selectedSupplier.pitch}</p>
                  </div>
                  <div className="w-full">
                    {renderDescription(selectedSupplier)}
                  </div>
                </div>
